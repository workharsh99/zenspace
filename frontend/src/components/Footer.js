      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            <div className="flex gap-3 mt-4">
              {['🧘', '🌿', '💆', '🌊'].map((emoji, i) => (
                <span key={i} className="text-2xl hover:scale-110 transition-transform cursor-default">{emoji}</span>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {[
                ...
                <li key={link.to}>
                  <Link to={link.to} className="hover:text-wellness-400 transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-slate-800 mt-10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-slate-500 text-sm">
            © 2024 ZenSpace. Built with ❤️ for mental wellness.
          </p>
