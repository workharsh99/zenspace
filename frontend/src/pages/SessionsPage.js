          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? 'bg-wellness-500 text-white shadow-sm'
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2">
                                <h3 className="font-semibold text-slate-800 truncate">
                                  {session.activityTitle || 'Wellness Session'}
                                </h3>
                                <span className={`badge ${sc.color} flex-shrink-0`}>
                                  {sc.icon} {session.status.replace('_', ' ')}
                                </span>
                              </div>
                              <div className="flex flex-wrap gap-3 mt-2 text-xs text-slate-400">
                                ...
                                {session.actualDurationMinutes > 0 && (
                                  <span>⏱️ {session.actualDurationMinutes} min</span>
                                )}
                                {session.moodBefore && (
                              ...
                              {session.notes && (
                                <p className="text-slate-500 text-sm mt-2 italic">"{session.notes}"</p>
                              )}
                            </div>
                          </div>
                        </div>
                    ...
                    {favorites.map(fav => (
                      <div key={fav.id} className="card overflow-hidden group">
                        ...
                        <div className="relative h-36 overflow-hidden">
                          ...
                          <button
                            onClick={() => handleRemoveFavorite(fav.activityId)}
