...
const wellnessGoals = [
  'Reduce stress and anxiety', 'Improve sleep quality', 'Build mindfulness habit',
  'Increase energy levels', 'Improve focus and productivity', 'Emotional balance',
];
...
const RegisterPage = () => {
  ...
  const validateStep1 = () => {
    if (!form.username || form.username.length < 3) { toast.error('Username must be at least 3 characters'); return false; }
    if (!form.email || !/\S+@\S+\.\S+/.test(form.email)) { toast.error('Please enter a valid email'); return false; }
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (errorData?.errors) Object.values(errorData.errors).forEach(msg => toast.error(msg));
      else toast.error(errorData?.error || 'Registration failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="card p-8 shadow-xl">
            ...
            {[1, 2].map(s => (
              <React.Fragment key={s}>
                <div className={`flex items-center gap-2 ${s <= step ? 'text-wellness-600' : 'text-slate-300'}`}>
                  ...
                  <span className="text-xs font-medium hidden sm:block">{s === 1 ? 'Account Info' : 'Wellness Goals'}</span>
                </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" name="firstName" value={form.firstName} onChange={handleChange} placeholder="John" className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Last Name</label>
                  <input type="text" name="lastName" value={form.lastName} onChange={handleChange} placeholder="Doe" className="input-field" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Username *</label>
                <input type="text" name="username" value={form.username} onChange={handleChange} placeholder="Choose a username" className="input-field" required />
              </div>
              <div>
              ...
              <button type="button" onClick={() => validateStep1() && setStep(2)} className="btn-primary w-full py-3.5 text-base mt-2">
                Continue →
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-3">What's your main wellness goal?</label>
                <div className="grid grid-cols-1 gap-2">
                  {wellnessGoals.map(goal => (
                    <label key={goal} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                      form.wellnessGoal === goal ? 'border-wellness-400 bg-wellness-50' : 'border-slate-200 hover:border-slate-300'
                    }`}>
                      <input type="radio" name="wellnessGoal" value={goal} checked={form.wellnessGoal === goal}
                        onChange={handleChange} className="text-wellness-500" />
              ...
              <div>
                ...
                <select name="stressLevel" value={form.stressLevel} onChange={handleChange} className="input-field">
          ...
          <div className="mt-6 text-center">
            <p className="text-slate-500 text-sm">
              ...
              <Link to="/login" className="text-wellness-600 font-semibold hover:text-wellness-700">Sign in</Link>
            </p>
          </div>
        </div>
